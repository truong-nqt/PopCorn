...
import com.truongnqt.popcorn.database.DatabaseHelper;
...
public class Favourite {
    ...
    public static List<TVShowBrief> getFavTVShowBriefs(Context context) {
        ...
        while (cursor.moveToNext()) {
            ...
            @SuppressLint("Range") String posterPath = cursor.getString(cursor.getColumnIndex(DatabaseHelper.POSTER_PATH));
            @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(DatabaseHelper.NAME));
            favTVShows.add(new TVShowBrief(null, tvShowId, name, null, null, posterPath, null, null, null, null, null, null, null, null));
        }
        cursor.close();
        database.close();
        return favTVShows;
